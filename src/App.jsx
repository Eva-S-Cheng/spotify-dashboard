...
export default function App(){
  ...
  const[evoData,setEvoData]=useState(null);const[evoL,setEvoL]=useState(false);const[evoView,setEvoView]=useState("artists");const[evoMode,setEvoMode]=useState("artists");
   ...
   try{
    ...
    const draw=x=>{
      ...
      periods.forEach(([lab,k],i)=>{const cy=hy+imgR;const a=aL(k)[0];const im=top1Imgs[i];
        ...
        hy+=rowH});
      hy+=12;
      if(stable.length){x.fillStyle="#B3FF5C";x.font="bold 26px monospace";x.fillText("PROGRESSION DE RANG",70,hy);hy+=34;
        const chL=stable.slice(0,6);const cx0=110,cx1=W-90,cy0=hy,chH=280;let maxR=1;chL.forEach(s=>PK.forEach(k=>{const r=rankOf(k,s.id);if(r&&r>maxR)maxR=r}));maxR=Math.min(Math.max(maxR,2),30);
        const px=i=>cx0+(cx1-cx0)*(i/3);const py=r=>cy0+chH*((r-1)/(maxR-1));
        x.strokeStyle="rgba(255,255,255,0.06)";x.lineWidth=1;[0,1,2,3].forEach(i=>{x.beginPath();x.moveTo(px(i),cy0);x.lineTo(px(i),cy0+chH);x.stroke()});
        x.fillStyle="#888";x.font="16px monospace";x.textAlign="center";["TOUT","6 MOIS","4 SEM","50 ÉC."].forEach((l,i)=>x.fillText(l,px(i),cy0+chH+28));x.textAlign="left";
        chL.forEach((s,si)=>{const col=CL[si%CL.length];x.strokeStyle=col;x.lineWidth=3.5;x.beginPath();let started=false;PK.forEach((k,i)=>{const r=rankOf(k,s.id);if(!r)return;const X=px(i),Y=py(Math.min(r,maxR));started?x.lineTo(X,Y):(x.moveTo(X,Y),started=true)});x.stroke();PK.forEach((k,i)=>{const r=rankOf(k,s.id);if(!r)return;x.fillStyle=col;x.beginPath();x.arc(px(i),py(Math.min(r,maxR)),5,0,Math.PI*2);x.fill()})});
        hy=cy0+chH+54;let lx=110;x.font="18px Inter,Arial,sans-serif";chL.forEach((s,si)=>{const col=CL[si%CL.length];const nm=trunc(x,s.name,220);const w=x.measureText(nm).width+34;if(lx+w>W-70){lx=110;hy+=32}x.fillStyle=col;x.beginPath();x.arc(lx+7,hy-6,7,0,Math.PI*2);x.fill();x.fillStyle="#ccc";x.fillText(nm,lx+22,hy);lx+=w+14});hy+=46}
  ...
  return(
    <div className="uc-fadein app-root" style={{background:"transparent",minHeight:"100vh",fontFamily:"'Inter',sans-serif",color:C.txt,padding:"20px 16px 100px",maxWidth:1500,margin:"0 auto"}}>
      ...
      {tab==="evolution"&&<div>
        ...
        {evoData&&(()=>{
          const genreRank=k=>{const c={};((evoData[k]?.artists)||[]).forEach(a=>(a.genres||[]).forEach(g=>{c[g]=(c[g]||0)+1}));return Object.entries(c).sort((a,b)=>b[1]-a[1]).map(([g])=>({id:g,name:g}))};
          ...
          return<>
            ...
            <Card style={{marginBottom:16}}><Lbl>Progression de rang — {picked.length} {isGmode?"genres constants":"valeurs sûres"}</Lbl><p style={{color:C.mut,fontSize:MQ?12:10,marginTop:-8,marginBottom:12}}>Position de tes {noun} dans le temps (plus haut = mieux classé), de Tout vers tes 50 dernières écoutes. <b style={{color:C.txt}}>Clique une ligne (ou la légende)</b> pour la mettre en avant.</p>
              {picked.length>0?<ResponsiveContainer width="100%" height={Math.max(360,Math.min(620,picked.length*26))}><LineChart data={chartData} margin={{left:0,right:20,top:8,bottom:0}}><XAxis dataKey="p" tick={{fill:C.txt,fontSize:MQ?14:12}} axisLine={false} tickLine={false} /><YAxis reversed allowDecimals={false} domain={[1,maxRank]} tick={{fill:C.mut,fontSize:MQ?12:10}} axisLine={false} tickLine={false} width={28} label={{value:"rang",angle:-90,position:"insideLeft",fill:C.mut,fontSize:MQ?12:10}} /><Tooltip cursor={{stroke:C.brd2,strokeWidth:1,strokeDasharray:"3 3"}} content={({active,payload,label})=>{if(!active||!payload||!payload.length)return null;const key=evoHover||evoSel;if(!key)return null;const it=payload.find(p=>p.dataKey===key);if(!it||it.value==null)return null;return<div style={{background:C.bg,border:`1px solid ${C.brd2}`,borderRadius:10,padding:"8px 12px",boxShadow:"0 10px 28px rgba(0,0,0,0.55)"}}><div style={{color:C.txt,fontWeight:700,fontSize:12}}>{it.name}</div><div style={{color:C.grnB,fontSize:12}}>{label} · #{it.value}</div></div>}} />{showLegend&&<Legend wrapperStyle={{fontSize:MQ?12:10,color:C.mut,cursor:"pointer"}} iconSize={8} onClick={o=>{setEvoSel(o.dataKey);showRankDrill(o.dataKey)}} />}{picked.map((pk,i)=>{const sel=evoSel===pk.id,hov=evoHover===pk.id,dim=(evoSel&&!sel)||(!evoSel&&evoHover&&!hov);return<Line key={pk.id} type="monotone" dataKey={pk.id} name={pk.name} stroke={CL[i%CL.length]} strokeWidth={sel?4:hov?3.4:2.2} strokeOpacity={dim?0.13:1} dot={dim?false:{r:sel?4:3}} activeDot={{r:6}} connectNulls style={{cursor:"pointer"}} onClick={()=>{setEvoSel(pk.id);showRankDrill(pk.id)}} onMouseEnter={()=>setEvoHover(pk.id)} onMouseLeave={()=>setEvoHover(null)} />})}</LineChart></ResponsiveContainer>:<p style={{color:C.mut,fontSize:MQ?14:12,textAlign:"center",padding:20}}>Pas de {isGmode?"genre":"artiste"} commun aux 3 périodes pour tracer la progression.</p>}
            </Card>
            ...
            <Card><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10,marginBottom:14}}><Lbl>Comparaison détaillée</Lbl><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{[["artists","Artistes"],["tracks","Titres"],["genres","Genres"]].map(([k,l])=><button key={k} onClick={()=>setEvoView(k)} style={{padding:"6px 14px",borderRadius:50,fontSize:MQ?13:11,background:evoView===k?C.grn:C.sf,border:`1px solid ${evoView===k?C.grn:C.brd}`,color:evoView===k?"#000":C.mut,cursor:"pointer",fontWeight:evoView===k?700:400}}>{l}</button>)}</div></div>
              {(()=>{const gbp={};["long","medium","short","recent"].forEach(kk=>{const c={};(evoData[kk]?.artists||[]).forEach(a=>(a.genres||[]).forEach(g=>{c[g]=(c[g]||0)+1}));gbp[kk]=c});return<div style={{display:"grid",gridTemplateColumns:MQ?"repeat(auto-fit,minmax(150px,1fr))":"repeat(auto-fit,minmax(170px,1fr))",gap:MQ?"28px 14px":12}}>{[["long","Tout le temps"],["medium","6 mois"],["short","4 semaines"],["recent","50 écoutes"]].map(([k,l])=>{const list=evoView==="genres"?Object.entries(gbp[k]||{}).map(([name,n])=>({id:name,name,count:n})).sort((a,b)=>b.count-a.count).slice(0,25):((evoData[k]||{})[evoView]||[]);return<div key={k}><div style={{color:C.acc,fontSize:MQ?13:11,fontFamily:"monospace",letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:8}}>{l}</div><div style={{maxHeight:480,overflowY:"auto"}}>{list.map((it,i)=>{
