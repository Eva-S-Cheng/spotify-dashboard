...
export default function App(){
  ...
  return(
    <div className="uc-fade" style={{background:BG,minHeight:"100vh",fontFamily:"'Inter',sans-serif",color:C.txt,padding:"20px 16px 100px",maxWidth:1200,margin:"0 auto"}}>
      {spin}
      {cur&&<><div onClick={closeDrill} style={{position:"fixed",top:0,left:0,width:"100vw",height:"100vh",background:"rgba(0,0,0,0.55)",backdropFilter:"blur(3px)",WebkitBackdropFilter:"blur(3px)",zIndex:999}} /><DrillDown title={cur.title} items={cur.content} onClose={closeDrill} onBack={popDrill} canBack={drillStack.length>1} /></>}

      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18,flexWrap:"wrap",gap:12}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>{prof.images?.[0]&&<div style={{position:"relative",flexShrink:0}}><div style={{position:"absolute",inset:-3,borderRadius:"50%",background:`conic-gradient(from 180deg,${C.grnB},${C.acc},${C.grn},${C.grnB})`,filter:"blur(0.5px)"}} /><img src={prof.images[0].url} alt="" style={{position:"relative",width:46,height:46,borderRadius:"50%",objectFit:"cover",border:`2px solid ${C.bg}`}} /></div>}<div><h1 style={{margin:0,fontSize:22,fontWeight:600,fontFamily:FD,letterSpacing:"-0.02em"}}>{prof.display_name}</h1><p style={{margin:0,color:C.mut,fontSize:11,letterSpacing:"0.02em"}}>{isPage?(pageTabs.find(([k])=>k===tab)||[])[1]:`Analyse · ${TL[tr]}`}</p></div></div>
      ...
      {!isPage&&<div style={{display:"flex",gap:4,marginBottom:14,borderBottom:`1px solid ${C.brd}`,overflowX:"auto"}}>{anaTabs.map(([k,l,recentOnly])=>{const disabled=recentOnly&&!isRecent;const on=tab===k;return<button key={k} disabled={disabled} onClick={()=>!disabled&&setTab(k)} title={disabled?"Disponible en mode « 50 écoutes »":""} style={{padding:"10px 13px",background:"none",border:"none",color:disabled?C.dim:on?C.grnB:C.mut,borderBottom:`2px solid ${on?C.grnB:"transparent"}`,cursor:disabled?"not-allowed":"pointer",fontSize:11.5,fontWeight:on?700:500,marginBottom:-1,whiteSpace:"nowrap",opacity:disabled?0.5:1,textShadow:on?"0 0 18px rgba(30,215,96,0.55)":"none"}}>{l}</button>})}</div>}
