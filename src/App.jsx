...
export default function App(){
  ...
  const[evoFrom,setEvoFrom]=useState("long");const[evoTo,setEvoTo]=useState("short");const[evoSel,setEvoSel]=useState(null);
  ...
  return(
    <div className="uc-fadein app-root" style={{background:"transparent",minHeight:"100vh",fontFamily:"'Inter',sans-serif",color:C.txt,padding:"20px 16px 100px",maxWidth:1500,margin:"0 auto"}}>
      ...
      {/* ÉVOLUTION */}
      {tab==="evolution"&&<div>
        <Card style={{marginBottom:16}}><Lbl>Évolution des goûts</Lbl><p style={{color:C.mut,fontSize:MQ?14:12,margin:0}}>Comparaison automatique de tes tops (30 artistes / 50 titres) sur les 3 périodes Spotify + tes 50 dernières écoutes.</p></Card>
        ...
        {evoData&&(()=>{
          ...
          return<>
            ...
            <Card style={{marginBottom:16}}><Lbl>Progression de rang — {picked.length} valeurs sûres</Lbl><p style={{color:C.mut,fontSize:MQ?12:10,marginTop:-8,marginBottom:12}}>Position dans tes tops à travers le temps (plus haut = mieux classé), de Tout vers tes 50 dernières écoutes. <b style={{color:C.txt}}>Clique une ligne (ou la légende)</b> pour la mettre en avant.</p>
              {evoSel&&idName[evoSel]&&<div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",marginBottom:12,padding:"10px 14px",borderRadius:12,background:"rgba(30,215,96,0.10)",border:`1px solid rgba(30,215,96,0.35)`}}><span style={{color:C.txt,fontWeight:700,fontSize:MQ?14:13}}>{idName[evoSel]}</span><a href={`https://open.spotify.com/artist/${evoSel}`} target="_blank" rel="noreferrer" style={{padding:"6px 14px",borderRadius:50,background:`linear-gradient(135deg,${C.grnB},${C.grn})`,color:"#04140A",fontSize:MQ?14:12,fontWeight:700,textDecoration:"none"}}>Ouvrir sur Spotify ↗</a><button onClick={()=>setEvoSel(null)} style={{padding:"6px 12px",borderRadius:50,background:"transparent",border:`1px solid ${C.brd}`,color:C.mut,fontSize:MQ?14:12,cursor:"pointer"}}>Tout réafficher</button></div>}
              {picked.length>0?<ResponsiveContainer width="100%" height={Math.max(360,Math.min(620,picked.length*26))}><LineChart data={chartData} margin={{left:0,right:20,top:8,bottom:0}}><XAxis dataKey="p" tick={{fill:C.txt,fontSize:MQ?14:12}} axisLine={false} tickLine={false} /><YAxis reversed allowDecimals={false} domain={[1,maxRank]} tick={{fill:C.mut,fontSize:MQ?12:10}} axisLine={false} tickLine={false} width={28} label={{value:"rang",angle:-90,position:"insideLeft",fill:C.mut,fontSize:MQ?12:10}} /><Tooltip contentStyle={{background:C.bg,border:`1px solid ${C.brd2}`,borderRadius:10,color:C.txt,fontSize:MQ?13:11,boxShadow:"0 10px 28px rgba(0,0,0,0.55)"}} itemSorter={i=>i.value==null?9999:i.value} formatter={(v,n)=>[`#${v}`,n]} />{showLegend&&<Legend wrapperStyle={{fontSize:MQ?12:10,color:C.mut,cursor:"pointer"}} iconSize={8} onClick={o=>setEvoSel(s=>s===o.dataKey?null:o.dataKey)} />}{picked.map((pk,i)=>{const sel=evoSel===pk.id,dim=evoSel&&!sel;return<Line key={pk.id} type="monotone" dataKey={pk.id} name={pk.name} stroke={CL[i%CL.length]} strokeWidth={sel?3.8:2.2} strokeOpacity={dim?0.12:1} dot={dim?false:{r:sel?4:3}} activeDot={{r:6}} connectNulls style={{cursor:"pointer"}} onClick={()=>setEvoSel(s=>s===pk.id?null:pk.id)} />})}</LineChart></ResponsiveContainer>:<p style={{color:C.mut,fontSize:MQ?14:12,textAlign:"center",padding:20}}>Pas d'artiste commun aux 3 périodes pour tracer la progression.</p>}
            </Card>
            ...
            <Card><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10,marginBottom:14}}><Lbl>Comparaison détaillée</Lbl><div style={{display:"flex",gap:6}}>{[["artists","Artistes"],["tracks","Titres"]].map(([k,l])=><button key={k} onClick={()=>setEvoView(k)} style={{padding:"6px 14px",borderRadius:50,fontSize:MQ?13:11,background:evoView===k?C.grn:C.sf,border:`1px solid ${evoView===k?C.grn:C.brd}`,color:evoView===k?"#000":C.mut,cursor:"pointer",fontWeight:evoView===k?700:400}}>{l}</button>)}</div></div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:12}}>{[["long","Tout le temps"],["medium","6 mois"],["short","4 semaines"],["recent","50 écoutes"]].map(([k,l])=>{const list=(evoData[k]||{})[evoView]||[];return<div key={k}><div style={{color:C.acc,fontSize:MQ?13:11,fontFamily:"monospace",letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:8}}>{l}</div><div style={{maxHeight:480,overflowY:"auto"}}>{list.map((it,i)=>{const inOthers=Object.entries(evoData).some(([kk,v])=>kk!==k&&((v||{})[evoView]||[]).some(z=>z.id===it.id));
