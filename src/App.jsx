  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Inter',sans-serif",color:C.txt,padding:"20px 16px 100px",maxWidth:1200,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18,flexWrap:"wrap",gap:12}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>{prof.images?.[0]&&<img src={prof.images[0].url} alt="" style={{width:44,height:44,borderRadius:"50%",objectFit:"cover",border:`2px solid ${C.grn}`}} />}<div><h1 style={{margin:0,fontSize:20,fontWeight:700}}>{prof.display_name}</h1><p style={{margin:0,color:C.mut,fontSize:11}}>{isPage?(pageTabs.find(([k])=>k===tab)||[])[1]:`Analyse · ${TL[tr]}`}</p></div></div>
        <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",justifyContent:"flex-end"}}>
          {Object.entries(TL).map(([k,l])=>{const on=!isPage&&tr===k;return<span key={k} style={{display:"inline-flex",alignItems:"center",gap:6}}>{k==="recent"&&<span style={{width:1,height:18,background:C.brd,margin:"0 1px"}} />}<button onClick={()=>changeTr(k)} style={{padding:"6px 13px",borderRadius:50,fontSize:11,background:on?C.grn:C.card,border:`1px solid ${on?C.grn:C.brd}`,color:on?"#000":C.mut,cursor:"pointer",fontWeight:on?700:400}}>{l}</button></span>})}
          <span style={{width:1,height:22,background:C.brd,margin:"0 4px"}} />
          {pageTabs.map(([k,l])=>{const on=tab===k;return<button key={k} onClick={()=>setTab(k)} style={{padding:"6px 13px",borderRadius:50,fontSize:11,background:on?C.acc:C.card,border:`1px solid ${on?C.acc:C.brd}`,color:on?"#000":C.mut,cursor:"pointer",fontWeight:on?700:500,whiteSpace:"nowrap"}}>{l}</button>})}
        </div>
      </div>
      <p style={{color:C.mut,fontSize:10,marginTop:-6,marginBottom:12}}>Choisis une <b style={{color:C.txt}}>période</b> (vert) pour l'analyse, ou un <b style={{color:C.acc}}>outil</b> (lime) indépendant de la période.</p>

      {!isPage&&<div style={{display:"flex",gap:2,marginBottom:14,borderBottom:`1px solid ${C.brd}`,overflowX:"auto"}}>{anaTabs.map(([k,l,recentOnly])=>{const disabled=recentOnly&&!isRecent;return<button key={k} disabled={disabled} onClick={()=>!disabled&&setTab(k)} title={disabled?"Disponible en mode « 50 écoutes »":""} style={{padding:"10px 12px",background:"none",border:"none",color:disabled?C.dim:tab===k?C.grn:C.mut,borderBottom:`2px solid ${tab===k?C.grn:"transparent"}`,cursor:disabled?"not-allowed":"pointer",fontSize:11,fontWeight:tab===k?600:400,marginBottom:-1,whiteSpace:"nowrap",opacity:disabled?0.5:1}}>{l}</button>})}</div>}
