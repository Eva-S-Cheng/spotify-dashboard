...
export default function App(){
  ...
  const[evoFrom,setEvoFrom]=useState("long");const[evoTo,setEvoTo]=useState("short");const[evoSel,setEvoSel]=useState(null);const[evoHover,setEvoHover]=useState(null);
  ...
  return(
    <div className="uc-fadein app-root" style={{background:"transparent",minHeight:"100vh",fontFamily:"'Inter',sans-serif",color:C.txt,padding:"20px 16px 100px",maxWidth:1500,margin:"0 auto"}}>
        ...
        {evoData&&(()=>{
          ...
          return<>
            ...
            <Card style={{marginBottom:16}}><Lbl>Progression de rang — {picked.length} valeurs sûres</Lbl><p style={{color:C.mut,fontSize:MQ?12:10,marginTop:-8,marginBottom:12}}>Position dans tes tops à travers le temps (plus haut = mieux classé), de Tout vers tes 50 dernières écoutes. <b style={{color:C.txt}}>Clique une ligne (ou la légende)</b> pour la mettre en avant.</p>
              ...
              {evoSel&&idName[evoSel]&&(()=>{const ranks=cols.map(([k])=>rankOf(k,evoSel));const valid=ranks.filter(r=>r);const best=valid.length?Math.min(...valid):null;const trend=(()=>{const first=ranks.find(r=>r),last=[...ranks].reverse().find(r=>r);if(!first||!last||first===last)return null;return first-last})();return<div style={{marginTop:16,borderRadius:16,overflow:"hidden",border:`1px solid rgba(30,215,96,0.35)`,boxShadow:"0 12px 34px -12px rgba(30,215,96,0.45)",background:"linear-gradient(135deg,rgba(30,215,96,0.14),rgba(45,212,191,0.05) 60%,rgba(0,0,0,0))"}}>
                <div style={{display:"flex",alignItems:"center",gap:14,padding:MQ?"14px":"16px 18px",flexWrap:"wrap"}}>
                  {idImg[evoSel]?<img src={idImg[evoSel]} alt="" style={{width:60,height:60,borderRadius:"50%",objectFit:"cover",border:`2px solid ${C.grn}`,boxShadow:"0 6px 18px rgba(0,0,0,0.5)",flexShrink:0}} />:<div style={{width:60,height:60,borderRadius:"50%",background:C.grn,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",color:"#04140A",fontWeight:800,fontFamily:FD,fontSize:24}}>{(idName[evoSel]||"?")[0]}</div>}
                  <div style={{flex:1,minWidth:140}}><div style={{color:C.txt,fontWeight:700,fontSize:MQ?19:20,fontFamily:FD,letterSpacing:"-0.02em",lineHeight:1.1}}>{idName[evoSel]}</div><div style={{color:C.mut,fontSize:MQ?12:11,marginTop:3}}>{best?<>Meilleur classement <b style={{color:C.grnB}}>#{best}</b></>:"—"}{trend!==null&&<span style={{color:trend>0?C.grn:C.red,fontWeight:700}}>{"  "}· {trend>0?`▲ +${trend}`:`▼ ${trend}`} depuis Tout</span>}</div></div>
                  <div style={{display:"flex",gap:8,flexShrink:0}}><a href={`https://open.spotify.com/artist/${evoSel}`} target="_blank" rel="noreferrer" style={{padding:"9px 16px",borderRadius:50,background:`linear-gradient(135deg,${C.grnB},${C.grn})`,color:"#04140A",fontSize:MQ?13:12,fontWeight:700,textDecoration:"none",whiteSpace:"nowrap"}}>Spotify ↗</a><button onClick={()=>setEvoSel(null)} style={{padding:"9px 12px",borderRadius:50,background:"rgba(0,0,0,0.25)",border:`1px solid ${C.brd2}`,color:C.mut,fontSize:MQ?14:13}}>✕</button></div>
                </div>
                <div style={{padding:MQ?"0 14px 14px":"0 18px 16px"}}><div style={{display:"flex",flexWrap:"wrap",alignItems:"stretch",gap:6}}>{cols.flatMap(([k,l],ci)=>{const r=rankOf(k,evoSel);const isBest=r&&r===best;const chip=<div key={"c"+k} style={{flex:"1 1 78px",minWidth:70,textAlign:"center",padding:"10px 6px",borderRadius:12,background:isBest?"rgba(30,215,96,0.16)":"rgba(0,0,0,0.30)",border:`1px solid ${isBest?"rgba(30,215,96,0.5)":C.brd}`}}><div style={{color:C.mut,fontSize:MQ?10.5:9.5,fontFamily:"monospace",letterSpacing:"0.04em",textTransform:"uppercase",marginBottom:5}}>{l}</div><div style={{color:r?(isBest?C.grnB:C.txt):C.dim,fontSize:MQ?20:19,fontWeight:700,fontFamily:FD}}>{r?`#${r}`:"—"}</div></div>;if(ci===0)return[chip];const pr=rankOf(cols[ci-1][0],evoSel);const d=(pr&&r)?pr-r:null;const ar=d===null?["·",C.dim]:d>0?["▲",C.grn]:d<0?["▼",C.red]:["=",C.mut];return[<div key={"a"+k} style={{flex:"0 0 auto",display:"flex",alignItems:"center",color:ar[1],fontSize:MQ?14:13,fontWeight:800}} title={d===null?"":d>0?`+${d} places`:d<0?`${d} places`:"stable"}>{ar[0]}</div>,chip]})}</div><p style={{color:C.mut,fontSize:MQ?11:9.5,marginTop:10,marginBottom:0}}>▲ progresse · ▼ recule · = stable, d'une période à la suivante.</p></div>
              </div>})()}
            </Card>
            ...
            <Card><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10,marginBottom:14}}><Lbl>Comparaison détaillée</Lbl><div style={{display:"flex",gap:6}}>{[["artists","Artistes"],["tracks","Titres"]].map(([k,l])=><button key={k} onClick={()=>setEvoView(k)} style={{padding:"6px 14px",borderRadius:50,fontSize:MQ?13:11,background:evoView===k?C.grn:C.sf,border:`1px solid ${evoView===k?C.grn:C.brd}`,color:evoView===k?"#000":C.mut,cursor:"pointer",fontWeight:evoView===k?700:400}}>{l}</button>)}</div></div>
              <div style={{display:"grid",gridTemplateColumns:MQ?"repeat(auto-fit,minmax(150px,1fr))":"repeat(auto-fit,minmax(170px,1fr))",gap:MQ?"28px 14px":12}}>{[["long","Tout le temps"],["medium","6 mois"],["short","4 semaines"],["recent","50 écoutes"]].map(([k,l])=>{const list=(evoData[k]||{})[evoView]||[];return<div key={k}><div style={{color:C.acc,fontSize:MQ?13:11,fontFamily:"monospace",letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:8}}>{l}</div><div style={{maxHeight:480,overflowY:"auto"}}>{list.map((it,i)=>{const inOthers=Object.entries(evoData).some(([kk,v])=>kk!==k&&((v||{})[evoView]||[]).some(z=>z.id===it.id));
              ...
              <p style={{color:C.mut,fontSize:MQ?12:10,marginTop:12}}>« NEW » = présent dans cette colonne mais dans aucune des autres. {evoView==="artists"&&"Clique un artiste pour voir ses titres récents et le nombre de lectures."}</p>
            </Card>
          </>;
        })()}
      </div>}
      ...
      {tab==="compat"&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:16}}>
        ...
        <Card><Lbl>Comparer avec un·e ami·e</Lbl><textarea value={friendCode} onChange={e=>setFriendCode(e.target.value)} placeholder="Colle ici le code de ton ami·e…" style={{width:"100%",height:90,resize:"none",padding:12,background:C.sf,border:`1px solid ${C.brd}`,borderRadius:10,color:C.txt,fontSize:MQ?13:11,fontFamily:"monospace",outline:"none",boxSizing:"border-box",wordBreak:"break-all"}} /><button onClick={compare} disabled={!friendCode.trim()} style={{marginTop:10,padding:"10px 20px",background:friendCode.trim()?C.grn:C.brd,border:"none",borderRadius:50,color:friendCode.trim()?"#000":C.mut,fontSize:MQ?14:12,fontWeight:700,cursor:friendCode.trim()?"pointer":"default"}}>Comparer</button>{compatErr&&<p style={{color:C.red,fontSize:MQ?14:12,marginTop:12}}>{compatErr}</p>}{compatRes&&<div style={{marginTop:16}}>{(()=>{const s=compatRes.score;const v=s>=75?["Jumeaux musicaux 👯",C.grnB]:s>=50?["Belle alchimie ✨",C.acc]:s>=25?["Atomes crochus",C.grn]:s>=8?["Goûts qui se frôlent","#FBBF24"]:["Deux mondes 🌍",C.red];const R=54,CC=2*Math.PI*R;return<div style={{textAlign:"center",marginBottom:18}}><div style={{position:"relative",width:140,height:140,margin:"0 auto"}}><svg width="140" height="140" style={{transform:"rotate(-90deg)"}}><circle cx="70" cy="70" r={R} fill="none" stroke="rgba(255,255,255,0.08)" strokeWidth="10"/><circle cx="70" cy="70" r={R} fill="none" stroke={v[1]} strokeWidth="10" strokeLinecap="round" strokeDasharray={CC} strokeDashoffset={CC*(1-s/100)} style={{transition:"stroke-dashoffset 0.7s ease",filter:`drop-shadow(0 0 6px ${v[1]})`}}/></svg><div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{fontSize:34,fontWeight:700,fontFamily:FD,color:C.txt,lineHeight:1}}>{s}%</div></div></div><div style={{color:v[1],fontWeight:700,fontSize:MQ?16:15,marginTop:8}}>{v[0]}</div><div style={{color:C.mut,fontSize:MQ?13:11,marginTop:2}}>avec {compatRes.name}</div></div>})()}{compatRes.sub&&<div style={{marginBottom:16}}>{[["Artistes",compatRes.sub.a],["Genres",compatRes.sub.g],["Pays",compatRes.sub.c]].map(([lab,val])=><div key={lab} style={{marginBottom:9}}><div style={{display:"flex",justifyContent:"space-between",fontSize:MQ?12:10,color:C.mut,marginBottom:3}}><span>{lab}</span><span style={{fontFamily:"monospace",color:C.grnB}}>{val}%</span></div><div style={{height:6,borderRadius:3,background:"rgba(255,255,255,0.06)"}}><div style={{height:6,borderRadius:3,width:`${val}%`,background:`linear-gradient(90deg,${C.grn},${C.grnB})`,transition:"width 0.6s ease"}} /></div></div>)}</div>}{[["Artistes en commun",compatRes.artists,C.grn],["Genres en commun",compatRes.genres,C.acc],["Pays en commun",compatRes.countries,C.grnB]].map(([lab,items,col])=>items.length>0?<div key={lab} style={{marginBottom:12}}><div style={{color:C.mut,fontSize:MQ?12:10,fontFamily:"monospace",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:8}}>{lab} ({items.length})</div><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{items.slice(0,40).map((it,i)=><span key={i} style={{padding:"4px 11px",borderRadius:50,background:"rgba(30,215,96,0.12)",border:`1px solid ${col}55`,color:C.txt,fontSize:MQ?12:11}}>{it}</span>)}</div></div>:null)}{compatRes.artists.length===0&&compatRes.genres.length===0&&compatRes.countries.length===0&&<p style={{color:C.mut,fontSize:MQ?14:12,textAlign:"center"}}>Aucun recouvrement — des goûts vraiment aux antipodes !</p>}</div>}</Card>
      </div>}
