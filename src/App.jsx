...
function DrillDown({title,artists,tracks,mb,onClose,onPlay,onMkPl}){
  return(<div style={{position:"fixed",top:0,right:0,width:480,maxWidth:"100vw",height:"100vh",background:C.card,borderLeft:`1px solid ${C.brd}`,zIndex:1000,overflowY:"auto",padding:24,boxSizing:"border-box"}}>
    ...
    {tracks.length>0&&<button onClick={()=>onMkPl(`${title} Mix`,tracks.slice(0,100).map(t=>t.uri))} style={{padding:"6px 14px",background:C.grn,border:"none",borderRadius:50,color:"#000",fontSize:11,fontWeight:600,cursor:"pointer",marginBottom:16}}>📋 Playlist ({Math.min(tracks.length,50)} titres)</button>}
