...
async function sp(e,t,o={}){const r=await fetch(`https://api.spotify.com/v1${e}`,{headers:{Authorization:`Bearer ${t}`,"Content-Type":"application/json"},...o});if(r.status===204)return null;if(!r.ok){const err=new Error(`${r.status}`);err.status=r.status;throw err}return r.json()}
...
// Alpha-2 → ISO numérique (pour matcher les géographies world-atlas, dont l'id est le code numérique)
...
// Genre filtering: keep compound genres, reject standalone generics
const STANDALONE=new Set(["metal","rock","pop","electronic","hip hop","hip-hop","jazz","classical","country","blues","folk","soul","punk","alternative","indie","r&b","rnb","dance","reggae","latin"]);
...
function isGenre(t){const l=t.toLowerCase().trim();if(ORIGINS.has(l)||STANDALONE.has(l))return false;return ROOTS.some(r=>l.includes(r))}
...
// Couleur d'un pays sur la carte selon son nombre d'artistes
...
export default function App(){
  ...
  const mbCache=useRef({});// PERSISTENT CACHE across time range changes
  ...
  useEffect(()=>{if(!tok)return;setLd(true);setErr(null);setLm("Récupération…");(async()=>{try{
    ...
    setLd(false);// ← Spotify s'affiche IMMÉDIATEMENT, MusicBrainz enrichit derrière

    // Affiche d'abord le cache, puis enrichit incrémentalement (chaque artiste met à jour l'UI)
    ...
    if(toFetch.length>0){
      ...
      for(const a of toFetch){
        ...
        if(r){mbCache.current[a.id]=r;setMb(prev=>({...prev,[a.id]:r}))}// MAJ live
        setMbP(p=>p+1);
  ...
  const mkPl=async(n,uris)=>{if(!uris?.length){alert("Aucun titre");return}try{const p=await sp(`/users/${data.prof.id}/playlists`,tok,{method:"POST",body:JSON.stringify({name:n,public:false})});if(!p?.id)throw new Error("Création échouée");for(let i=0;i<uris.length;i+=100)await sp(`/playlists/${p.id}/tracks`,tok,{method:"POST",body:JSON.stringify({uris:uris.slice(i,i+100)})});alert(`"${n}" — ${uris.length} titres !`)}catch(e){if(e.status===403){setPlErr("Spotify a refusé (403). Ton autorisation date d'avant l'ajout des permissions playlist. Révoque l'app puis reconnecte-toi.")}else{alert("Erreur "+e.message)}}};
  ...
  const exportJSON=()=>dl(`spotify-${tr}.json`,JSON.stringify({profil:prof.display_name,periode:TL[tr],obscurite:obscurity,genres:allG,pays:allC,artistes:tA.map(a=>({nom:a.name,popularite:a.popularity,genres:mb[a.id]?.genres||[],pays:mb[a.id]?.country||null}))},null,2),"application/json");
  const exportCSV=()=>{const rows=[["#","Artiste","Popularité","Genres","Pays"]];tA.forEach((a,i)=>{const m=mb[a.id];rows.push([i+1,a.name,a.popularity??"",(m?.genres||[]).join(" / "),m?.country||""])});dl(`spotify-artistes-${tr}.csv`,"\uFEFF"+rows.map(r=>r.map(c=>`"${String(c).replace(/"/g,'""')}"`).join(",")).join("\n"),"text/csv")};
  const generateShare=()=>{const W=1080,H=1350,cv=document.createElement("canvas");cv.width=W;cv.height=H;const x=cv.getContext("2d");
    x.fillStyle="#0D0D0D";x.fillRect(0,0,W,H);
    x.fillStyle="#1DB954";x.font="bold 70px Inter,sans-serif";x.fillText("Your Spotify",80,150);
    x.fillStyle="#fff";x.fillText("Uncovered",80,230);
    x.fillStyle="#888";x.font="28px monospace";x.fillText(`${prof.display_name} · ${TL[tr]}`,80,285);
    x.fillStyle="#B3FF5C";x.font="bold 30px monospace";x.fillText("TOP 5 ARTISTES",80,400);
    tA.slice(0,5).forEach((a,i)=>{const y=475+i*72;x.fillStyle="#1DB954";x.font="bold 38px monospace";x.fillText(`${i+1}`,80,y);x.fillStyle="#fff";x.font="40px Inter,sans-serif";x.fillText(a.name.slice(0,26),150,y)});
    const yy=920;
    if(allG[0]){x.fillStyle="#B3FF5C";x.font="bold 28px monospace";x.fillText("GENRE #1",80,yy);x.fillStyle="#fff";x.font="bold 52px Inter,sans-serif";x.fillText(allG[0].name,80,yy+62)}
    if(allC[0]){x.fillStyle="#B3FF5C";x.font="bold 28px monospace";x.fillText("PAYS #1",80,yy+170);x.fillStyle="#fff";x.font="bold 52px Inter,sans-serif";x.fillText(allC[0].name,80,yy+232)}
    if(obscurity!==null){x.fillStyle="#B3FF5C";x.font="bold 28px monospace";x.fillText("OBSCURITÉ",600,yy);x.fillStyle="#fff";x.font="bold 52px Inter,sans-serif";x.fillText(`${obscurity}/100`,600,yy+62)}
    x.fillStyle="#555";x.font="22px monospace";x.fillText("eva-s-cheng.github.io/spotify-dashboard",80,H-60);
  ...
  const compare=()=>{setCompatErr(null);setCompatRes(null);if(!friendCode.trim())return;try{
    ...
    const lc=arr=>new Set((arr||[]).map(s=>String(s).toLowerCase()));
    const jac=(s1,s2)=>{const inter=[...s1].filter(x=>s2.has(x));const u=new Set([...s1,...s2]);return{score:u.size?inter.length/u.size:0,inter}};
    const sA1=lc(myCompat.a),sA2=lc(f.a);const aj=jac(sA1,sA2);
    const gj=jac(lc(myCompat.g),lc(f.g));const cj=jac(lc(myCompat.c),lc(f.c));
    const score=Math.round((aj.score*0.5+gj.score*0.3+cj.score*0.2)*100);
    const cap=s=>s.charAt(0).toUpperCase()+s.slice(1);
    setCompatRes({name:f.n||"ton ami·e",score,
      artists:aj.inter.map(cap),genres:gj.inter.map(cap),countries:cj.inter.map(cap)});
  ...
  const openDrill=(title,artists)=>{
    ...
    setDrill({title,content:<div>
      ...
      {artists.map((a,i)=>{const m=mb[a.id];const at=tracks.filter(t=>(t.artists||[]).some(ta=>ta.id===a.id));return<div key={a.id} style={{marginBottom:10}}><div style={{display:"flex",alignItems:"center",gap:10,padding:"5px 0"}}><span style={{color:C.mut,fontSize:10,width:20,textAlign:"right",fontFamily:"monospace"}}>{i+1}</span>{a.images?.[0]?<img src={a.images[a.images.length>1?1:0].url} alt="" style={{width:34,height:34,borderRadius:"50%",objectFit:"cover"}} />:<div style={{width:34,height:34,borderRadius:"50%",background:C.dim}} />}<div style={{flex:1}}><div style={{color:C.txt,fontSize:13,fontWeight:600}}>{a.name}</div>{m&&<div style={{color:C.mut,fontSize:10}}>{(m.genres||[]).join(", ")}{m.country?` · ${m.country}`:""}</div>}</div></div>{at.length>0&&<div style={{marginLeft:64}}>{at.map(t=><div key={t.id} style={{display:"flex",alignItems:"center",gap:8,padding:"2px 0",cursor:"pointer",borderBottom:`1px solid ${C.brd}`}} onClick={()=>play(t.uri)}><div style={{flex:1,minWidth:0}}><div style={{color:C.txt,fontSize:11,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.name}</div></div><span style={{color:C.grn,fontSize:10}}>▶</span></div>)}</div>}</div>})}
    </div>});
  };
    ...
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Inter',sans-serif",color:C.txt,padding:"20px 16px 100px",maxWidth:1200,margin:"0 auto"}}>
      ...
      {tab==="overview"&&<>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:12,marginBottom:20}}>
          ...
          {allG.length>0&&<SC label="Genre #1" value={allG[0].name} sub={`${allG[0].count} artistes`} icon="🎨" onClick={()=>openDrill(allG[0].name,abg[allG[0].name]||[])} />}
          ...
          <SC label="Genres" value={allG.length} icon="🏷" onClick={()=>setDrill({title:"Tous les genres",content:<div>{allG.map((g,i)=><div key={g.name} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderBottom:`1px solid ${C.brd}`,cursor:"pointer"}} onClick={()=>openDrill(g.name,abg[g.name]||[])}><div style={{width:8,height:8,borderRadius:"50%",background:CL[i%CL.length]}} /><div style={{flex:1,color:C.txt,fontSize:12}}>{g.name}</div><span style={{color:C.mut,fontSize:10}}>{g.count}</span></div>)}</div>})} />
          <SC label="Pays" value={allC.length} icon="🗺" onClick={()=>setDrill({title:"Tous les pays",content:<div>{allC.map((c,i)=><div key={c.name} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 0",borderBottom:`1px solid ${C.brd}`,cursor:"pointer"}} onClick={()=>openDrill(c.name,abc[c.name]||[])}><div style={{width:8,height:8,borderRadius:"50%",background:CL[i%CL.length]}} /><div style={{flex:1,color:C.txt,fontSize:13}}>{c.name}</div><span style={{color:C.mut,fontSize:10}}>{c.count}</span></div>)}</div>})} />
        </div>
      ...
      {tab==="genres"&&(allG.length>0?<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}><Card><Lbl>Genres ({allG.length})</Lbl><ResponsiveContainer width="100%" height={Math.max(340,pieG.length*18)}><PieChart><Pie data={pieG} dataKey="count" nameKey="name" cx="50%" cy="50%" outerRadius={Math.min(140,50+pieG.length*3)} innerRadius={50} paddingAngle={1} isAnimationActive={false} label={({name,percent})=>percent>0.03?name:""} labelLine={false} onClick={d=>{if(d.name!=="Autre"&&abg[d.name])openDrill(d.name,abg[d.name])}}>{pieG.map((_,i)=><Cell key={i} fill={CL[i%CL.length]} style={{cursor:"pointer"}} />)}</Pie><Tooltip contentStyle={{background:C.card,border:`1px solid ${C.brd}`,borderRadius:8,color:C.txt}} formatter={(v,n)=>[`${v} artistes`,n]} /></PieChart></ResponsiveContainer></Card><Card><Lbl>Tous ({allG.length})</Lbl><div style={{maxHeight:600,overflowY:"auto"}}>{allG.map((g,i)=><div key={g.name} style={{display:"flex",alignItems:"center",gap:10,padding:"5px 0",borderBottom:`1px solid ${C.brd}`,cursor:"pointer"}} onClick={()=>openDrill(g.name,abg[g.name]||[])}><div style={{width:8,height:8,borderRadius:"50%",background:CL[i%CL.length],flexShrink:0}} /><div style={{flex:1,color:C.txt,fontSize:12}}>{g.name}</div><span style={{color:C.mut,fontSize:10,fontFamily:"monospace"}}>{g.count}</span></div>)}</div></Card></div>:<Card><p style={{color:C.mut,textAlign:"center"}}>{mbL?`Enrichissement ${mbP}/${mbT}`:"Pas de données"}</p></Card>)}

      {/* CARTE DU MONDE (choroplèthe) */}
      {tab==="countries"&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <Card style={{position:"relative"}}><Lbl>Carte ({allC.length} pays)</Lbl>
              ...
              <ZoomableGroup center={[10,15]} zoom={1} maxZoom={6}>
                <Geographies geography={WORLD_TOPO}>
                  {({geographies})=>geographies.map(geo=>{
                    ...
                    const code=NUM_TO_A2[num];
                    const count=code?(ccByCode[code]||0):0;
                    return <Geography key={geo.rsmKey} geography={geo}
                      fill={mapColor(count,maxC)} stroke={C.bg} strokeWidth={0.3}
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,marginTop:10}}>
            <span style={{color:C.mut,fontSize:10}}>0</span>
            <div style={{flex:1,height:8,borderRadius:4,background:`linear-gradient(90deg,${C.sf},${mapColor(maxC/2,maxC)},${C.grn})`}} />
            <span style={{color:C.mut,fontSize:10}}>{maxC} artistes</span>
          </div>
          <p style={{color:C.mut,fontSize:10,marginTop:6}}>Survole un pays pour le détail · clique pour voir les artistes · zoom à la molette.</p>
        </Card>
        <Card><Lbl>Tous ({allC.length})</Lbl><div style={{maxHeight:600,overflowY:"auto"}}>{allC.map((c,i)=><div key={c.name} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 0",borderBottom:`1px solid ${C.brd}`,cursor:"pointer"}} onClick={()=>openDrill(c.name,abc[c.name]||[])}><div style={{width:8,height:8,borderRadius:"50%",background:CL[i%CL.length],flexShrink:0}} /><div style={{flex:1,color:C.txt,fontSize:13}}>{c.name}</div><span style={{color:C.mut,fontSize:10,fontFamily:"monospace"}}>{c.count}</span></div>)}</div></Card>
      </div>}

      {tab==="trends"&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}><Card><Lbl>Écoutes / heure</Lbl><ResponsiveContainer width="100%" height={250}><BarChart data={hr}><XAxis dataKey="h" tick={{fill:C.mut,fontSize:10}} axisLine={false} tickLine={false} interval={2} /><YAxis tick={{fill:C.mut,fontSize:10}} axisLine={false} tickLine={false} /><Tooltip contentStyle={{background:C.card,border:`1px solid ${C.brd}`,borderRadius:8,color:C.txt}} /><Bar dataKey="nb" fill={C.grn} radius={[4,4,0,0]} name="Écoutes" /></BarChart></ResponsiveContainer></Card><Card><Lbl>Minutes / heure</Lbl><ResponsiveContainer width="100%" height={250}><BarChart data={hr}><XAxis dataKey="h" tick={{fill:C.mut,fontSize:10}} axisLine={false} tickLine={false} interval={2} /><YAxis tick={{fill:C.mut,fontSize:10}} axisLine={false} tickLine={false} /><Tooltip contentStyle={{background:C.card,border:`1px solid ${C.brd}`,borderRadius:8,color:C.txt}} formatter={v=>[`${Math.round(v)} min`]} /><Bar dataKey="min" fill={C.acc} radius={[4,4,0,0]} /></BarChart></ResponsiveContainer></Card><Card style={{gridColumn:"span 2"}}><Lbl>Genre dominant / heure</Lbl><p style={{color:C.mut,fontSize:10,marginTop:-8,marginBottom:10}}>Pour chaque heure où tu as écouté, le genre le plus présent. La longueur de barre = nombre d'écoutes. Survole pour le genre exact.</p><ResponsiveContainer width="100%" height={Math.max(200,gph.length*28)}><BarChart data={gph} layout="vertical"><XAxis type="number" tick={{fill:C.mut,fontSize:10}} axisLine={false} tickLine={false} /><YAxis type="category" dataKey="h" tick={{fill:C.grn,fontSize:11,fontWeight:700}} axisLine={false} tickLine={false} width={40} /><Tooltip contentStyle={{background:C.card,border:`1px solid ${C.brd}`,borderRadius:8,color:C.txt}} formatter={(v,n,p)=>[`${v} écoutes — ${p.payload.genre}`]} /><Bar dataKey="nb" radius={[0,6,6,0]}>{gph.map((d,i)=><Cell key={i} fill={CL[hGenres.indexOf(d.genre)%CL.length]||C.dim} />)}</Bar></BarChart></ResponsiveContainer><div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:8}}>{hGenres.map((g,i)=><div key={g} style={{display:"flex",alignItems:"center",gap:4,cursor:"pointer"}} onClick={()=>openDrill(g,abg[g]||[])}><div style={{width:8,height:8,borderRadius:"50%",background:CL[i%CL.length]}} /><span style={{color:C.mut,fontSize:10}}>{g}</span></div>)}</div></Card></div>}

      {tab==="discover"&&<div><Card style={{marginBottom:16}}><Lbl>Suggestions automatiques</Lbl>{sugL&&<p style={{color:C.mut,fontSize:11}}>Recherche en cours…</p>}{sug.length===0&&!sugL&&<p style={{color:C.mut,fontSize:12}}>Disponible après enrichissement.</p>}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:12}}>{sug.map((s,si)=><Card key={si} style={{padding:16,background:C.sf}}><div style={{marginBottom:10}}><span style={{color:C.acc,fontSize:13,fontWeight:600}}>{s.genre}</span>{s.country&&<span style={{color:C.mut,fontSize:10}}> · {s.country}</span>}</div>{s.artists.map((a,ai)=><div key={ai} style={{display:"flex",alignItems:"center",gap:10,padding:"5px 0",borderBottom:`1px solid ${C.brd}`,cursor:a.sid?"pointer":"default"}} onClick={()=>{if(a.sid)playArt(a.sid)}}>{a.img?<img src={a.img} alt="" style={{width:34,height:34,borderRadius:"50%",objectFit:"cover"}} />:<div style={{width:34,height:34,borderRadius:"50%",background:CL[(si*5+ai)%CL.length],display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:"#000",fontWeight:700}}>{a.name[0]}</div>}<div style={{flex:1,minWidth:0}}><div style={{color:C.txt,fontSize:12,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.name}</div><div style={{color:C.mut,fontSize:9}}>{(a.tags||[]).join(", ")}</div></div>{a.sid&&<span style={{color:C.grn,fontSize:11}}>▶</span>}</div>)}</Card>)}</div></Card><Card><Lbl>Recherche personnalisée</Lbl><div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}><select value={cG} onChange={e=>setCG(e.target.value)} style={{flex:1,minWidth:140,padding:10,background:C.sf,border:`1px solid ${C.brd}`,borderRadius:8,color:C.txt,fontSize:12,outline:"none"}}><option value="">Genre…</option>{allG.map(g=><option key={g.name} value={g.name}>{g.name} ({g.count})</option>)}</select><select value={cC} onChange={e=>setCC(e.target.value)} style={{flex:1,minWidth:140,padding:10,background:C.sf,border:`1px solid ${C.brd}`,borderRadius:8,color:C.txt,fontSize:12,outline:"none"}}><option value="">Tous pays</option>{allC.map(c=><option key={c.name} value={c.name}>{c.name}</option>)}</select><button onClick={customSearch} disabled={!cG||cL2} style={{padding:"10px 20px",background:cG?C.grn:C.brd,border:"none",borderRadius:8,color:cG?"#000":C.mut,fontSize:12,fontWeight:600,cursor:cG?"pointer":"default"}}>{cL2?"…":"Chercher"}</button></div>{cR.map((s,si)=><div key={si}><div style={{color:C.acc,fontSize:13,fontWeight:600,marginBottom:8}}>{s.genre}{s.country?` · ${s.country}`:""}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>{s.artists.map((a,ai)=><div key={ai} style={{display:"flex",alignItems:"center",gap:10,padding:8,background:C.sf,borderRadius:10,cursor:a.sid?"pointer":"default",border:`1px solid ${C.brd}`}} onClick={()=>{if(a.sid)playArt(a.sid)}}>{a.img?<img src={a.img} alt="" style={{width:38,height:38,borderRadius:"50%",objectFit:"cover"}} />:<div style={{width:38,height:38,borderRadius:"50%",background:CL[ai%CL.length],display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,color:"#000",fontWeight:700}}>{a.name[0]}</div>}<div style={{flex:1,minWidth:0}}><div style={{color:C.txt,fontSize:12,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.name}</div><div style={{color:C.mut,fontSize:9}}>{(a.tags||[]).join(", ")}</div></div>{a.sid&&<span style={{color:C.grn}}>▶</span>}</div>)}</div></div>)}</Card></div>}

      {/* PLAYLISTS — lien Spotify, lecture visible, plus de drill-down */}
      {tab==="playlists"&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}><Card><Lbl>Mes playlists ({pls.length})</Lbl><div style={{maxHeight:700,overflowY:"auto"}}>{pls.map(p=>{const isPlaying=nowUri===p.uri;const n=typeof p.tracks?.total==="number"?p.tracks.total:null;return<div key={p.id} style={{display:"flex",alignItems:"center",gap:12,padding:isPlaying?"8px":"8px 0",borderBottom:`1px solid ${C.brd}`,background:isPlaying?"rgba(29,185,84,0.1)":"transparent",borderRadius:isPlaying?10:0,marginBottom:isPlaying?4:0}}>
        ...
        <div style={{flex:1,minWidth:0}}><div style={{color:isPlaying?C.grn:C.txt,fontSize:12,fontWeight:isPlaying?700:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.name}</div><div style={{color:C.mut,fontSize:10}}>{n!==null?`${n} titres`:"—"}{isPlaying&&<span style={{color:C.grn,fontWeight:700}}> · En lecture</span>}</div></div>
      ...
      </div>})}</div></Card><Card><Lbl>Créer des playlists</Lbl>{allG.slice(0,3).map(g=><button key={g.name} onClick={()=>{const a=abg[g.name]||[];const u=tT.filter(t=>(t.artists||[]).some(ar=>a.some(x=>x.id===ar.id))).map(t=>t.uri);mkPl(`Best of ${g.name}`,u)}} style={{display:"block",width:"100%",padding:"12px 16px",background:C.sf,border:`1px solid ${C.brd}`,borderRadius:10,color:C.txt,fontSize:12,cursor:"pointer",marginBottom:8,textAlign:"left"}}>🎨 Best of {g.name}</button>)}{allC.slice(0,3).map(c=><button key={c.name} onClick={()=>{const a=abc[c.name]||[];const u=tT.filter(t=>(t.artists||[]).some(ar=>a.some(x=>x.id===ar.id))).map(t=>t.uri);mkPl(`Best of ${c.name}`,u)}} style={{display:"block",width:"100%",padding:"12px 16px",background:C.sf,border:`1px solid ${C.brd}`,borderRadius:10,color:C.txt,fontSize:12,cursor:"pointer",marginBottom:8,textAlign:"left"}}>🌍 Best of {c.name}</button>)}</Card></div>}
      ...
      {/* LECTEUR — icônes SVG + tracklist album */}
      {tab==="player"&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}><Card>{np?<div><div style={{display:"flex",gap:16,marginBottom:20}}>{np.album?.images?.[0]&&<img src={np.album.images[0].url} alt="" style={{width:140,height:140,borderRadius:12}} />}<div style={{flex:1}}><div style={{fontSize:18,fontWeight:700,marginBottom:4}}>{np.name}</div><div style={{color:C.mut,fontSize:13}}>{(np.artists||[]).map(a=>a.name).join(", ")}</div><a href={`https://open.spotify.com/album/${np.album?.id}`} target="_blank" rel="noreferrer" style={{color:C.acc,fontSize:11,textDecoration:"none",display:"block",marginTop:4}}>💿 {np.album?.name} ↗</a>{np.album?.release_date&&<div style={{color:C.mut,fontSize:10,marginTop:2}}>📅 {np.album.release_date.slice(0,4)}</div>}{npMb&&<div style={{marginTop:6}}>{(npMb.genres||[]).length>0&&<div style={{color:C.acc,fontSize:11}}>🎨 {npMb.genres.join(", ")}</div>}{npMb.country&&<div style={{color:C.acc,fontSize:11}}>🌍 {npMb.country}</div>}</div>}{ctxName&&<div style={{marginTop:6,padding:"4px 10px",background:C.sf,borderRadius:6,display:"inline-block"}}><span style={{color:C.grn,fontSize:11,fontWeight:600}}>{pl?.context?.type==="playlist"?"📋":"💿"} {ctxName}</span></div>}{pl?.progress_ms&&np.duration_ms&&<div style={{marginTop:10}}><div style={{width:"100%",height:4,background:C.brd,borderRadius:2}}><div style={{width:`${(pl.progress_ms/np.duration_ms)*100}%`,height:"100%",background:C.grn,borderRadius:2,transition:"width 1s linear"}} /></div><div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><span style={{color:C.mut,fontSize:10,fontFamily:"monospace"}}>{fmt(pl.progress_ms/60000)}</span><span style={{color:C.mut,fontSize:10,fontFamily:"monospace"}}>{fmt(np.duration_ms/60000)}</span></div></div>}</div></div><div style={{display:"flex",justifyContent:"center",alignItems:"center",gap:10}}><PB label="Aléatoire" icon={I.shuffle()} active={pl?.shuffle_state} onClick={()=>cmd("shuffle")} /><PB label="Précédent" icon={I.prev()} onClick={()=>cmd("prev")} /><PB label={pl?.is_playing?"Pause":"Lecture"} icon={pl?.is_playing?I.pause():I.play()} big onClick={()=>cmd(pl?.is_playing?"pause":"play")} /><PB label="Suivant" icon={I.next()} onClick={()=>cmd("next")} /><PB label="Répéter" icon={pl?.repeat_state==="track"?I.repeatOne():I.repeat()} active={pl?.repeat_state!=="off"} onClick={()=>cmd("repeat")} /></div></div>:<div style={{textAlign:"center",padding:40}}><p style={{color:C.mut,fontSize:14}}>Ouvre Spotify sur un appareil</p></div>}</Card><div style={{display:"flex",flexDirection:"column",gap:16}}><Card><Lbl>Appareils ({devs.length})</Lbl>{devs.length>0?devs.map(d=><div key={d.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:`1px solid ${C.brd}`}}><span style={{fontSize:18}}>{d.type==="Computer"?"💻":d.type==="Smartphone"?"📱":"🔊"}</span><div style={{flex:1}}><div style={{color:C.txt,fontSize:13,fontWeight:500}}>{d.name}</div><div style={{color:C.mut,fontSize:10}}>{d.type} · Vol. {d.volume_percent}%</div></div>{d.is_active&&<div style={{background:C.grn,color:"#000",fontSize:9,fontWeight:700,padding:"2px 8px",borderRadius:50}}>ACTIF</div>}</div>):<p style={{color:C.mut}}>Aucun appareil</p>}</Card>{np&&albumTks.length>0&&<Card><Lbl>Album · {np.album?.name}</Lbl><div style={{maxHeight:300,overflowY:"auto"}}>{albumTks.map((t,i)=>{const cur=t.id===np.id;return<div key={t.id} onClick={()=>play(t.uri)} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderBottom:`1px solid ${C.brd}`,cursor:"pointer"}}><span style={{color:cur?C.grn:C.mut,fontSize:10,width:20,textAlign:"right",fontFamily:"monospace"}}>{i+1}</span><div style={{flex:1,minWidth:0,color:cur?C.grn:C.txt,fontSize:12,fontWeight:cur?700:400,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.name}</div><span style={{color:C.mut,fontSize:10,fontFamily:"monospace"}}>{t.duration_ms?`${Math.floor(t.duration_ms/60000)}:${String(Math.floor((t.duration_ms%60000)/1000)).padStart(2,"0")}`:""}</span></div>})}</div></Card>}</div></div>}

      {/* COMPATIBILITÉ MUSICALE */}
      {tab==="compat"&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <Card><Lbl>Ton code de profil</Lbl>
          <p style={{color:C.mut,fontSize:12,marginBottom:12}}>Envoie ce code à un·e ami·e qui utilise la même app. Il compare vos goûts (top artistes, genres, pays). Aucune donnée ne quitte vos navigateurs.</p>
          <textarea readOnly value={myCode} onFocus={e=>e.target.select()} style={{width:"100%",height:90,resize:"none",padding:12,background:C.sf,border:`1px solid ${C.brd}`,borderRadius:10,color:C.mut,fontSize:11,fontFamily:"monospace",outline:"none",boxSizing:"border-box",wordBreak:"break-all"}} />
          <button onClick={copyCode} style={{marginTop:10,padding:"10px 20px",background:copied?C.acc:C.grn,border:"none",borderRadius:50,color:"#000",fontSize:12,fontWeight:700,cursor:"pointer"}}>{copied?"Copié ✓":"Copier mon code"}</button>
        </Card>
        <Card><Lbl>Comparer avec un·e ami·e</Lbl>
          <textarea value={friendCode} onChange={e=>setFriendCode(e.target.value)} placeholder="Colle ici le code de ton ami·e…" style={{width:"100%",height:90,resize:"none",padding:12,background:C.sf,border:`1px solid ${C.brd}`,borderRadius:10,color:C.txt,fontSize:11,fontFamily:"monospace",outline:"none",boxSizing:"border-box",wordBreak:"break-all"}} />
          <button onClick={compare} disabled={!friendCode.trim()} style={{marginTop:10,padding:"10px 20px",background:friendCode.trim()?C.grn:C.brd,border:"none",borderRadius:50,color:friendCode.trim()?"#000":C.mut,fontSize:12,fontWeight:700,cursor:friendCode.trim()?"pointer":"default"}}>Comparer</button>
          {compatErr&&<p style={{color:C.red,fontSize:12,marginTop:12}}>{compatErr}</p>}
          {compatRes&&<div style={{marginTop:16}}>
            <div style={{textAlign:"center",marginBottom:16}}><div style={{fontSize:48,fontWeight:800,color:C.grn,fontFamily:"monospace",lineHeight:1}}>{compatRes.score}%</div><div style={{color:C.mut,fontSize:12,marginTop:4}}>de compatibilité avec {compatRes.name}</div></div>
            {compatRes.artists.length>0&&<div style={{marginBottom:12}}><div style={{color:C.acc,fontSize:11,fontFamily:"monospace",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>Artistes en commun ({compatRes.artists.length})</div><div style={{color:C.txt,fontSize:12,lineHeight:1.6}}>{compatRes.artists.join(", ")}</div></div>}
            {compatRes.genres.length>0&&<div style={{marginBottom:12}}><div style={{color:C.acc,fontSize:11,fontFamily:"monospace",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>Genres en commun</div><div style={{color:C.txt,fontSize:12,lineHeight:1.6}}>{compatRes.genres.join(", ")}</div></div>}
            {compatRes.countries.length>0&&<div><div style={{color:C.acc,fontSize:11,fontFamily:"monospace",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6}}>Pays en commun</div><div style={{color:C.txt,fontSize:12,lineHeight:1.6}}>{compatRes.countries.join(", ")}</div></div>}
            {compatRes.artists.length===0&&compatRes.genres.length===0&&compatRes.countries.length===0&&<p style={{color:C.mut,fontSize:12,textAlign:"center"}}>Aucun recouvrement détecté — des goûts très différents !</p>}
          </div>}
        </Card>
      </div>}
