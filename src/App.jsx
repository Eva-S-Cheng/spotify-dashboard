...
export default function App(){
  ...
  const openNames=(title,items)=>{const content=<div><p style={{color:C.mut,fontSize:MQ?14:12,marginBottom:12}}>{items.length} artiste(s){items.length?" — clique pour voir leurs titres récents":""}</p>{items.length?items.map((it,i)=><div key={(it.id||i)+""} onClick={()=>it.id&&openEvoArtist(it.name,it.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 0",borderBottom:`1px solid ${C.brd}`,cursor:it.id?"pointer":"default"}}><span style={{color:C.mut,fontSize:MQ?12:10,width:22,textAlign:"right",fontFamily:"monospace"}}>{i+1}</span><div style={{flex:1,minWidth:0,color:C.txt,fontSize:MQ?14:12,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{it.name}</div>{it.note&&<span style={{color:C.acc,fontSize:MQ?12:10,fontFamily:"monospace"}}>{it.note}</span>}</div>):<p style={{color:C.mut,fontSize:MQ?14:12}}>Aucun</p>}</div>;pushDrill(title,content)};
    ...
    <div className="uc-fadein app-root" style={{background:"transparent",minHeight:"100vh",fontFamily:"'Inter',sans-serif",color:C.txt,padding:"20px 16px 100px",maxWidth:1500,margin:"0 auto"}}>
        ...
        {evoData&&(()=>{
          ...
          return<>
            ...
            <Card style={{marginBottom:16}}><Lbl>Progression de rang — {picked.length} valeurs sûres</Lbl><p style={{color:C.mut,fontSize:MQ?12:10,marginTop:-8,marginBottom:12}}>Position dans tes tops à travers le temps (plus haut = mieux classé), de Tout vers tes 50 dernières écoutes. <b style={{color:C.txt}}>Clique une ligne (ou la légende)</b> pour la mettre en avant.</p>
              {picked.length>0?<ResponsiveContainer width="100%" height={Math.max(360,Math.min(620,picked.length*26))}><LineChart data={chartData} margin={{left:0,right:20,top:8,bottom:0}}><XAxis dataKey="p" tick={{fill:C.txt,fontSize:MQ?14:12}} axisLine={false} tickLine={false} /><YAxis reversed allowDecimals={false} domain={[1,maxRank]} tick={{fill:C.mut,fontSize:MQ?12:10}} axisLine={false} tickLine={false} width={28} label={{value:"rang",angle:-90,position:"insideLeft",fill:C.mut,fontSize:MQ?12:10}} /><Tooltip cursor={{stroke:C.brd2,strokeWidth:1,strokeDasharray:"3 3"}} content={({active,payload,label})=>{if(!active||!payload||!payload.length)return null;const key=evoHover||evoSel;if(!key)return null;const it=payload.find(p=>p.dataKey===key);if(!it||it.value==null)return null;return<div style={{background:C.bg,border:`1px solid ${C.brd2}`,borderRadius:10,padding:"8px 12px",boxShadow:"0 10px 28px rgba(0,0,0,0.55)"}}><div style={{color:C.txt,fontWeight:700,fontSize:12}}>{it.name}</div><div style={{color:C.grnB,fontSize:12}}>{label} · #{it.value}</div></div>}} />{showLegend&&<Legend wrapperStyle={{fontSize:MQ?12:10,color:C.mut,cursor:"pointer"}} iconSize={8} onClick={o=>{setEvoSel(o.dataKey);showRankDrill(o.dataKey)}} />}{picked.map((pk,i)=>{const sel=evoSel===pk.id,hov=evoHover===pk.id,dim=(evoSel&&!sel)||(!evoSel&&evoHover&&!hov);return<Line key={pk.id} type="monotone" dataKey={pk.id} name={pk.name} stroke={CL[i%CL.length]} strokeWidth={sel?4:hov?3.4:2.2} strokeOpacity={dim?0.13:1} dot={dim?false:{r:sel?4:3}} activeDot={{r:6}} connectNulls style={{cursor:"pointer"}} onClick={()=>{setEvoSel(pk.id);showRankDrill(pk.id)}} onMouseEnter={()=>setEvoHover(pk.id)} onMouseLeave={()=>setEvoHover(null)} />})}</LineChart></ResponsiveContainer>:<p style={{color:C.mut,fontSize:MQ?14:12,textAlign:"center",padding:20}}>Pas d'artiste commun aux 3 périodes pour tracer la progression.</p>}
            </Card>
      ...
      {tab==="player"&&<div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:16,alignItems:"start"}}>
          ...
          {np&&artAlbums.length>0&&<Card><Lbl>Autres albums de {np.artists?.[0]?.name}</Lbl><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:12,marginTop:4}}>{artAlbums.map(al=><div key={al.id} onClick={()=>withDevice({context_uri:al.uri})} title={`Lire ${al.name}`} style={{cursor:"pointer"}}>{al.images?.[0]?<img src={al.images[0].url} alt="" style={{width:"100%",aspectRatio:"1",borderRadius:10,objectFit:"cover",boxShadow:"0 6px 18px rgba(0,0,0,0.4)"}} />:<div style={{width:"100%",aspectRatio:"1",borderRadius:10,background:C.sf}} />}<div style={{color:C.txt,fontSize:MQ?12:11,fontWeight:500,marginTop:6,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{al.name}</div><div style={{color:C.mut,fontSize:MQ?11:9.5}}>{(al.release_date||"").slice(0,4)}</div></div>)}</div></Card>}
          <Card><Lbl>Appareils ({devs.length})</Lbl>{devs.length>0?devs.map(d=><div key={d.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:`1px solid ${C.brd}`}}><span style={{fontSize:18}}>{d.type==="Computer"?"💻":d.type==="Smartphone"?"📱":"🔊"}</span><div style={{flex:1}}><div style={{color:C.txt,fontSize:13,fontWeight:500}}>{d.name}</div><div style={{color:C.mut,fontSize:MQ?12:10}}>{d.type} · Vol. {d.volume_percent}%</div></div>{d.is_active&&<div style={{background:C.grn,color:"#000",fontSize:MQ?11:9,fontWeight:700,padding:"2px 8px",borderRadius:50}}>ACTIF</div>}</div>):<p style={{color:C.mut}}>Aucun appareil</p>}</Card>
        </div>
      </div>}
